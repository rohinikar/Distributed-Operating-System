...
object Project3_Bonus extends App{
  ...
  var maxRequests = 0
   ...
   def receive = {
     case MaxReached =>
       {
         max_reached_nodes += 1
         if(max_reached_nodes == (nodes.length-2))
         {
    ...
    def failNode()
    {
      if(nodes.length == n)
      {
        ...
        //nodes_ref((random_node.toInt)) ! fail
       context.stop(nodes_ref(random_node))
      }
    }
    ...
    case Sending(message : Message) =>
        {
        	  ...
        	  Project3.net ! MsgReached(message.no_of_hops+totalNumOfHops)
    ...
    case routeMessage(message : Message, destNodeId : String) =>
          {
            ...
            else if(destNodeId.toInt < nodeId.toInt)
            {
              var i : Int = 0
              var id : String = null
